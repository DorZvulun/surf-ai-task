...
| 8 | Write `infra/apps.tf`, `infra/variables.tf`, `infra/terraform.tfvars` — define both apps, instantiate module via `for_each`. `variables.tf` must include `variable "docker_username" {}`. `apps.tf` constructs python-app image as `"${var.docker_username}/ironman-web-app:latest"`; echo-app uses the public image directly from tfvars. `docker_username` is never in tfvars — it comes via `TF_VAR_docker_username` env var. | `terraform plan` shows correct resources | 20m | 7 |
...
| # | Task | Output | Time | Depends on |
|---|---|---|---|---|
| 11 | Write `Makefile` — all targets: `build`, `init`, `plan`, `apply`, `destroy`, `test`, `all`. Must start with `include .secrets` + `export` so all targets get `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN`, and `TF_VAR_docker_username` automatically from `.secrets`. `build` target uses `$(DOCKERHUB_USERNAME)/ironman-web-app:latest`. | `make all` runs end-to-end without error | 20m | 10 |
